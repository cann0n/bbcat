...
/**
 * Created by ${Rhino} on 2016/1/19 12:00
 */
public class MyFollowActivity extends SimpleActivity {
    ...
    public void initBusiness() {
        new TitleBar(activity).back().setTitle("我的关注");
        adapter = new FollowAdapter(activity, new ArrayList<FollowVo>());
    ...
    private void queryData(String id) {
        ...
        HttpUtils.postJSONObject(activity, Const.MY_FOLLOWS, SimpleUtils.buildUrl(activity, params), new RespJSONObjectListener(activity) {
            public void getResp(JSONObject jsonObject) {
                dismissDialog();
                follow_list.setRefreshing(false);
                RespVo<FollowVo> respVo = GsonTools.getVo(jsonObject.toString(), RespVo.class);
                if (respVo.isSuccess()) {
                    List<FollowVo> data = respVo.getListData(jsonObject, FollowVo.class);
                    if (pageNum == 1) {
                        adapter.removeAll();
