...
public class BodyFeaturesActivity extends SimpleActivity implements View.OnClickListener
{
    ...
    public void initBusiness()
    {
        ...
        if(state != null)
            state = new GrowthVo.BobyState();
        if(Utils.isEmpty(state.getMin_height()))
            state.setMin_height("40");
        if(Utils.isEmpty(state.getMax_height()))
            state.setMax_height("100");
        if(Utils.isEmpty(state.getMin_weight()))
            state.setMin_weight("1");
        if(Utils.isEmpty(state.getMax_weight()))
            state.setMax_weight("5");
        if(Utils.isEmpty(state.getMin_head()))
            state.setMin_head("10");
        if(Utils.isEmpty(state.getMax_head()))
            state.setMax_head("30");
