...
public class PersonInfosActivity extends SimpleActivity implements View.OnClickListener {
    ...
    public void initBusiness() {
        new TitleBar(activity).setTitle("个人信息").back().showRight("提交", new View.OnClickListener() {
            public void onClick(View v) {
                ...
                try {
                    ...
                    if (mSex.getText().toString().trim().equals("男")) params.put("sex", "1");
                    else if (mSex.getText().toString().trim().equals("女")) params.put("sex", "2");
        ...
        if (mLoginBean.getUserInfo().getSex().equals("2")) mSex.setText("女");
        else if (mLoginBean.getUserInfo().getSex().equals("1")) mSex.setText("男");
        else mSex.setText("未设定");
        if (mLoginBean.getUserInfo().getBaby_status().equals("1")) mState.setText("怀孕中");
        else if (mLoginBean.getUserInfo().getBaby_status().equals("2")) mState.setText("备孕中");
        else if (mLoginBean.getUserInfo().getBaby_status().equals("3")) mState.setText("已出生");
        else mState.setText("未设定");
...
//            alertDialog.getWindow().setAttributes(params);
            ...
            if (mSex.getText().toString().trim().equals("男")) intent.putExtra("sex", "1");
            else if (mSex.getText().toString().trim().equals("女")) intent.putExtra("sex", "2");
            else intent.putExtra("sex", "3");
            ...
            if (mState.getText().toString().trim().equals("怀孕中")) intent.putExtra("state", "1");
            else if (mState.getText().toString().trim().equals("备孕中"))
                intent.putExtra("state", "2");
            else if (mState.getText().toString().trim().equals("已出生"))
                intent.putExtra("state", "3");
            else intent.putExtra("state", "4");
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (data != null) {
            ...
            switch (requestCode) {
                case NICKNAME:
                    mNick.setText(data.getStringExtra("nickname"));
                    break;
                case SEX: {
                    if (data.getStringExtra("sex").equals("1")) mSex.setText("男");
                    else if (data.getStringExtra("sex").equals("2")) mSex.setText("女");
                }
                break;

                case STATE: {
                    if (data.getStringExtra("state").equals("1")) mState.setText("怀孕中");
                    else if (data.getStringExtra("state").equals("2")) mState.setText("备孕中");
                    else if (data.getStringExtra("state").equals("3")) mState.setText("已出生");
                }
                break;
            }
        }
